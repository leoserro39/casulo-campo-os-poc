...
REQ_TAG = "product-controlled-pilot-manual-dry-run-session-execution-plan-readiness-gate-v0.1"

PREV_OUT = ROOT / "outputs/prod4661_4700_controlled_pilot_manual_dry_run_session_execution_plan_readiness_gate.json"
...
ROADMAP_JSON = ROOT / "outputs/prod4661_4700_controlled_pilot_manual_dry_run_session_roadmap_snapshot.json"
...
DOC = ROOT / "docs/product/619_CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_MANUAL_EXECUTION_HOLD_PACKET.md"
CONTRACT = ROOT / "product/contracts/controlled_pilot_manual_dry_run_session_manual_execution_hold_packet.contract.json"
SPEC = ROOT / "product/memory/controlled_pilot_manual_dry_run_session_manual_execution_hold_packet_v0_1.json"
...
OUT_JSON = ROOT / "outputs/prod4701_4740_controlled_pilot_manual_dry_run_session_manual_execution_hold_packet.json"
OUT_MD = ROOT / "outputs/prod4701_4740_controlled_pilot_manual_dry_run_session_manual_execution_hold_packet.md"
ROADMAP_OUT = ROOT / "outputs/prod4701_4740_controlled_pilot_manual_dry_run_session_roadmap_snapshot.json"

HOLD_SECTIONS = [
    "hold_context_refs",
    "prior_gate_chain_review",
    "execution_plan_readiness_review",
    "execution_plan_packet_review",
    ...
    "manual_execution_block",
    "start_command_block",
    "operator_identity_placeholder",
    "human_reviewer_placeholder",
    "source_reference_boundary",
    "privacy_review_boundary",
    "pii_redaction_boundary",
    "secret_scan_boundary",
    "evidence_boundary",
    "abort_trigger_boundary",
    "blocked_action_boundary",
    "claim_boundary",
    "dataset_hold_boundary",
    "human_release_required",
    "post_hold_next_gate"
]

HOLD_FIELDS = [
    "manual_execution_hold_packet_id",
    "phase",
    "execution_plan_readiness_gate_ref",
    "execution_plan_packet_ref",
    "execution_plan_template_ref",
    "execution_plan_checklist_ref",
    ...
    "start_command_hold_reason",
    "manual_execution_hold_reason",
    "human_release_required",
    "hold_decision",
    "hold_reason",
    "next_allowed_phase",
...
HOLD_CHECKS = [
    "prior_execution_plan_readiness_gate_passed",
    ...
    "roadmap_updated",
    "hold_readiness_gate_preparation_only",
    "manual_execution_hold_review_before_release_required",
    "human_release_gate_required_before_session"
]

ALLOWED = [
    "manual_execution_hold_packet_creation",
    "manual_execution_hold_template_creation",
    "manual_execution_hold_checklist_creation",
    "roadmap_update",
...
def main():
    ...
    plan_gate = read_json(PLAN_READINESS_GATE) if PLAN_READINESS_GATE.exists() else {}
    ...
    for item in roadmap_items:
        item = dict(item)
        if item.get("phase") == "PROD-4661..4700":
            item["status"] = "DONE"
        elif item.get("phase") == PHASE:
            item["status"] = "CURRENT"
        elif item.get("phase") == "PROD-4741..4780":
            item["status"] = "NEXT"
        updated_items.append(item)

    if not any(item.get("phase") == PHASE for item in updated_items):
        updated_items.append({"phase": PHASE, "name": "Manual Session Execution Hold Packet", "status": "CURRENT"})
    if not any(item.get("phase") == "PROD-4741..4780" for item in updated_items):
        updated_items.append({"phase": "PROD-4741..4780", "name": "Manual Session Execution Hold Readiness Gate", "status": "NEXT"})

    hold_packet = {
        "version": "controlled_pilot_manual_dry_run_session_manual_execution_hold_packet.v0.1",
        "phase": PHASE,
        "purpose": "Create a hard hold packet before any controlled manual session execution.",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "manual_execution_hold_packet_only": True,
        "session_execution_not_performed": True,
        "start_command_executed": False,
        "manual_session_execution_performed": False,
        "hold_sections": HOLD_SECTIONS,
        ...
        "hold_boundary": {
            ...
            "human_release_required_before_execution": True
        },
        "claim_boundary": "Manual execution hold packet only. No session execution, no start command, no capture, no insert, no dataset acceptance.",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": "PROD-4741..4780 - Controlled Pilot Manual Dry Run Session Manual Session Execution Hold Readiness Gate"
    }

    hold_template = {
        "version": "controlled_pilot_manual_dry_run_session_manual_execution_hold_template.v0.1",
        "phase": PHASE,
        "status": "MANUAL_EXECUTION_HOLD_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "fields": {
        ...
        "default_gate": "HOLD_MANUAL_EXECUTION_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }

    hold_checklist = {
        "version": "controlled_pilot_manual_dry_run_session_manual_execution_hold_checklist.v0.1",
        "phase": PHASE,
        ...
        "real_candidate_accepted_to_dataset": False,
        "checks": [
            {"id": f"HOLD-{i+1:03d}", "name": name, "required": True, "status": "PENDING"}
            for i, name in enumerate(HOLD_CHECKS)
        ],
        "default_gate": "HOLD_MANUAL_EXECUTION_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }
    ...
    roadmap_out = {
        "version": "controlled_pilot_manual_dry_run_session_roadmap.v0.3",
        "phase": PHASE,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "roadmap_items": updated_items,
        "current_phase": "PROD-4701..4740 - Controlled Pilot Manual Dry Run Session Manual Session Execution Hold Packet",
        "next_phase": hold_packet["recommended_next_phase"],
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "blocked_actions": BLOCKED
    ...
    spec = {
        "version": "controlled_pilot_manual_dry_run_session_manual_execution_hold_packet.v0.1",
        "phase": PHASE,
        "purpose": hold_packet["purpose"],
        "hold_section_count": len(HOLD_SECTIONS),
        "hold_field_count": len(HOLD_FIELDS),
        "hold_check_count": len(HOLD_CHECKS),
        ...
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": hold_packet["recommended_next_phase"]
    }

    contract = {
        "phase": PHASE,
        "required_prior_tag": REQ_TAG,
        "mode": "controlled_pilot_manual_dry_run_session_manual_execution_hold_packet",
        "manual_execution_hold_packet_only": True,
        "session_execution_not_performed": True,
    ...
    doc = """# PROD-4701..4740 - Controlled Pilot Manual Dry Run Session Manual Session Execution Hold Packet

Creates the hard hold packet before any controlled manual dry run session execution.
...
Boundary: manual execution hold packet only. Any real/manual execution remains blocked until a later explicit human release gate.
"""
    ...
    roadmap_md += [
        "",
        "## Current boundary",
        "- Current phase creates a hard hold packet before execution.",
        "- No session execution.",
        "- No start command.",
        "- No real candidate insert.",
        "- No dataset acceptance.",
        "- Human release remains required before any actual execution."
    ]
    ...
    write_json(HOLD_PACKET, hold_packet)
    write_json(HOLD_TEMPLATE, hold_template)
    write_json(HOLD_CHECKLIST, hold_checklist)
    ...
    checks = {
        ...
        "previous_decision_hold_packet_prep_only": prev.get("decision") == "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_HOLD_PACKET_PREPARATION_ONLY",
        ...
        "plan_readiness_check_count": count_list(plan_gate, "readiness_checks"),
        ...
        "hold_field_count": len(HOLD_FIELDS),
        "hold_check_count": len(HOLD_CHECKS),
        "hold_template_field_count": len(hold_template["fields"]),
        "hold_checklist_item_count": len(hold_checklist["checks"]),
        "hold_packet_only": hold_packet["manual_execution_hold_packet_only"] is True,
        "session_execution_not_performed": hold_packet["session_execution_not_performed"] is True,
        "start_command_executed_false": hold_packet["start_command_executed"] is False,
        "manual_session_execution_performed_false": hold_packet["manual_session_execution_performed"] is False,
        "hold_no_real_data": hold_packet["real_data_captured_in_this_phase"] is False,
        "hold_no_candidate_insert": hold_packet["real_candidate_inserted"] is False,
        "hold_no_dataset_acceptance": hold_packet["real_candidate_accepted_to_dataset"] is False,
        "start_command_not_allowed": hold_packet["hold_boundary"]["start_command_allowed"] is False,
        "manual_session_execution_not_allowed": hold_packet["hold_boundary"]["manual_session_execution_allowed"] is False,
        "human_release_required": hold_packet["hold_boundary"]["human_release_required_before_execution"] is True,
        "contract_hold_only": contract["manual_execution_hold_packet_only"] is True,
        ...
        "roadmap_next_phase_present": any(item.get("phase") == "PROD-4741..4780" and item.get("status") == "NEXT" for item in updated_items),
    ...
    result = {
        "status": status,
        "phase": PHASE,
        "decision": "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_MANUAL_EXECUTION_HOLD_PACKET_READY" if status == "PASS" else "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_MANUAL_EXECUTION_HOLD_PACKET_NOT_READY",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "hold_section_count": len(HOLD_SECTIONS),
        "hold_field_count": len(HOLD_FIELDS),
        "hold_check_count": len(HOLD_CHECKS),
    ...
    report = [
        "# PROD-4701..4740 Controlled Pilot Manual Dry Run Session Manual Execution Hold Packet",
        "",
        f"- Status: `{status}`",
        f"- Decision: `{result['decision']}`",
        f"- Hold sections: `{len(HOLD_SECTIONS)}`",
        f"- Hold fields: `{len(HOLD_FIELDS)}`",
        f"- Hold checks: `{len(HOLD_CHECKS)}`",
        ...
        f"- Next: `{hold_packet['recommended_next_phase']}`",
        "",
        "## Boundary",
        "- Manual execution hold packet only.",
        "- No start command executed.",
        "- No session execution performed.",
        "- No real candidate inserted.",
        "- No dataset acceptance.",
        "- Human release required before any actual execution.",
        "",
        "## Errors"
    ]
    ...
    print("hold_sections:", len(HOLD_SECTIONS))
    print("hold_fields:", len(HOLD_FIELDS))
    print("hold_checks:", len(HOLD_CHECKS))
    ...
    print("next:", hold_packet["recommended_next_phase"])
    print("errors:", errors)
