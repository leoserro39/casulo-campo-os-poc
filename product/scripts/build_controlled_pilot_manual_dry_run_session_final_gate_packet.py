...
PHASE = "PROD-4381..4420"
REQ_TAG = "product-controlled-pilot-manual-dry-run-session-review-readiness-gate-v0.1"

PREV_OUT = ROOT / "outputs/prod4341_4380_controlled_pilot_manual_dry_run_session_review_readiness_gate.json"
...
DOC = ROOT / "docs/product/611_CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET.md"
CONTRACT = ROOT / "product/contracts/controlled_pilot_manual_dry_run_session_final_gate_packet.contract.json"
SPEC = ROOT / "product/memory/controlled_pilot_manual_dry_run_session_final_gate_packet_v0_1.json"
...
OUT_JSON = ROOT / "outputs/prod4381_4420_controlled_pilot_manual_dry_run_session_final_gate_packet.json"
OUT_MD = ROOT / "outputs/prod4381_4420_controlled_pilot_manual_dry_run_session_final_gate_packet.md"

FINAL_GATE_SECTIONS = [
    "final_gate_context_refs",
    ...
    "evidence_boundary",
    "human_reviewer_boundary",
    "operator_check_boundary",
...
FINAL_GATE_CHECKS = [
    "prior_review_readiness_gate_passed",
    ...
    "final_gate_decision_required",
    "final_gate_reason_required",
...
ALLOWED = [
    "final_gate_packet_creation",
    "final_gate_template_creation",
    "final_gate_checklist_creation",
    "sanitized_final_gate_placeholder_creation",
...
def main():
    ...
    review_packet_src = read_json(REVIEW_PACKET) if REVIEW_PACKET.exists() else {}
    review_template_src = read_json(REVIEW_TEMPLATE) if REVIEW_TEMPLATE.exists() else {}
    review_checklist_src = read_json(REVIEW_CHECKLIST) if REVIEW_CHECKLIST.exists() else {}
    ...
    review_sections = review_packet_src.get("review_sections", [])
    review_fields = review_packet_src.get("review_fields", [])
    review_checks = review_packet_src.get("review_checks", [])
    review_template_fields = review_template_src.get("fields", {})
    review_checklist_items = review_checklist_src.get("checks", [])
    ...
    final_packet = {
        "version": "controlled_pilot_manual_dry_run_session_final_gate_packet.v0.1",
        "phase": PHASE,
        "purpose": "Prepare controlled manual dry run session final gate packet without executing a session.",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "final_gate_packet_only": True,
        "session_execution_not_performed": True,
        "final_gate_sections": FINAL_GATE_SECTIONS,
        "final_gate_fields": FINAL_GATE_FIELDS,
        ...
        "claim_boundary": "Final gate packet only. No session execution, no capture, no insert, no dataset acceptance.",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": "PROD-4421..4460 - Controlled Pilot Manual Dry Run Session Final Gate Readiness Gate"
    }

    final_template = {
        "version": "controlled_pilot_manual_dry_run_session_final_gate_template.v0.1",
        "phase": PHASE,
        "status": "FINAL_GATE_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "fields": {
            ...
            for field in FINAL_GATE_FIELDS
        },
        "default_gate": "HOLD_FINAL_GATE_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }

    final_checklist = {
        "version": "controlled_pilot_manual_dry_run_session_final_gate_checklist.v0.1",
        "phase": PHASE,
        ...
        "real_candidate_accepted_to_dataset": False,
        "checks": [
            {"id": f"FINAL-{i+1:03d}", "name": name, "required": True, "status": "PENDING"}
            for i, name in enumerate(FINAL_GATE_CHECKS)
        ],
        "default_gate": "HOLD_FINAL_GATE_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }
    ...
    spec = {
        ...
        "final_gate_packet": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_final_gate_packet_v0_1.json",
        "final_gate_template": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_final_gate_template_v0_1.json",
        "final_gate_checklist": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_final_gate_checklist_v0_1.json",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": final_packet["recommended_next_phase"]
    }

    contract = {
        "phase": PHASE,
        "required_prior_tag": REQ_TAG,
        "mode": "controlled_pilot_manual_dry_run_session_final_gate_packet",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "final_gate_packet_only": True,
    ...
    doc = """# PROD-4381..4420 - Controlled Pilot Manual Dry Run Session Final Gate Packet

Creates the controlled manual dry run session final gate packet.
...
Boundary: final gate packet only. Use reference-only or sanitized placeholders. No automatic capture, raw private data, secrets, unredacted PII, production activation, client-facing claim, commercial pricing claim or validated real-world claim.
"""
    ...
    write_json(FINAL_TEMPLATE, final_template)
    write_json(FINAL_CHECKLIST, final_checklist)

    checks = {
        "required_tag_present": REQ_TAG in tags(),
        "previous_output_exists": PREV_OUT.exists(),
        "previous_output_pass": prev.get("status") == "PASS",
        "previous_decision_final_gate_prep_only": prev.get("decision") == "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET_PREPARATION_ONLY",
        ...
        "final_gate_field_count": len(FINAL_GATE_FIELDS),
        "final_gate_check_count": len(FINAL_GATE_CHECKS),
        "final_gate_template_field_count": len(final_template["fields"]),
        "final_gate_checklist_item_count": len(final_checklist["checks"]),
        "final_gate_packet_only": final_packet["final_gate_packet_only"] is True,
        "session_execution_not_performed": final_packet["session_execution_not_performed"] is True,
        "final_gate_no_real_data": final_packet["real_data_captured_in_this_phase"] is False,
        "final_gate_no_candidate_insert": final_packet["real_candidate_inserted"] is False,
        "final_gate_no_dataset_acceptance": final_packet["real_candidate_accepted_to_dataset"] is False,
        "contract_packet_only": contract["final_gate_packet_only"] is True,
    ...
    result = {
        "status": status,
        "phase": PHASE,
        "decision": "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET_READY" if status == "PASS" else "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET_NOT_READY",
    ...
    report = [
        "# PROD-4381..4420 Controlled Pilot Manual Dry Run Session Final Gate Packet",
        "",
        f"- Status: `{status}`",
        ...
        f"- Next: `{final_packet['recommended_next_phase']}`",
        "",
        "## Boundary",
        "- Final gate packet only.",
    ...
    print("next:", final_packet["recommended_next_phase"])
    print("errors:", errors)
