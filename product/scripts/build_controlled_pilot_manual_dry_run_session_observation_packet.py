...
PHASE = "PROD-4221..4260"
REQ_TAG = "product-controlled-pilot-manual-dry-run-session-execution-log-readiness-gate-v0.1"

PREV_OUT = ROOT / "outputs/prod4181_4220_controlled_pilot_manual_dry_run_session_execution_log_readiness_gate.json"
...
DOC = ROOT / "docs/product/607_CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OBSERVATION_PACKET.md"
CONTRACT = ROOT / "product/contracts/controlled_pilot_manual_dry_run_session_observation_packet.contract.json"
SPEC = ROOT / "product/memory/controlled_pilot_manual_dry_run_session_observation_packet_v0_1.json"
...
OUT_JSON = ROOT / "outputs/prod4221_4260_controlled_pilot_manual_dry_run_session_observation_packet.json"
OUT_MD = ROOT / "outputs/prod4221_4260_controlled_pilot_manual_dry_run_session_observation_packet.md"

OBSERVATION_SECTIONS = [
    "session_context_refs",
    "operator_reviewer_refs",
    "execution_log_refs",
    ...
    "manual_observations_sanitized",
    "operator_check_summary",
    "abort_trigger_summary",
...
OBSERVATION_CHECKS = [
    "prior_log_readiness_gate_passed",
    "log_shell_present",
...
ALLOWED = [
    "observation_packet_creation",
    "observation_template_creation",
    "observation_checklist_creation",
    "sanitized_manual_observation_placeholder_creation",
...
def main():
    ...
    obs_packet = {
        "version": "controlled_pilot_manual_dry_run_session_observation_packet.v0.1",
        "phase": PHASE,
        "purpose": "Prepare controlled manual dry run session observation packet without executing a session.",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "observation_packet_only": True,
        ...
        "claim_boundary": "Observation packet only. No session execution, no capture, no insert, no dataset acceptance.",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": "PROD-4261..4300 - Controlled Pilot Manual Dry Run Session Observation Readiness Gate"
    }

    obs_template = {
        "version": "controlled_pilot_manual_dry_run_session_observation_template.v0.1",
        "phase": PHASE,
        "status": "OBSERVATION_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "fields": {
        ...
        "default_gate": "HOLD_OBSERVATION_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }

    obs_checklist = {
        "version": "controlled_pilot_manual_dry_run_session_observation_checklist.v0.1",
        "phase": PHASE,
        ...
        "real_candidate_accepted_to_dataset": False,
        "checks": [
            {"id": f"OBS-{i+1:03d}", "name": name, "required": True, "status": "PENDING"}
            for i, name in enumerate(OBSERVATION_CHECKS)
        ],
        "default_gate": "HOLD_OBSERVATION_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }
    ...
    spec = {
        ...
        "observation_packet": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_observation_packet_v0_1.json",
        "observation_template": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_observation_template_v0_1.json",
        "observation_checklist": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_observation_checklist_v0_1.json",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": obs_packet["recommended_next_phase"]
    }

    contract = {
        "phase": PHASE,
        "required_prior_tag": REQ_TAG,
        "mode": "controlled_pilot_manual_dry_run_session_observation_packet",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "observation_packet_only": True,
    ...
    doc = """# PROD-4221..4260 - Controlled Pilot Manual Dry Run Session Observation Packet

Creates the controlled manual dry run session observation packet.
...
Boundary: observation packet only. Use reference-only or sanitized placeholders. No automatic capture, raw private data, secrets, unredacted PII, production activation, client-facing claim, commercial pricing claim or validated real-world claim.
"""

    write(DOC, doc)
    write_json(CONTRACT, contract)
    write_json(SPEC, spec)
    write_json(OBS_PACKET, obs_packet)
    write_json(OBS_TEMPLATE, obs_template)
    write_json(OBS_CHECKLIST, obs_checklist)

    checks = {
        "required_tag_present": REQ_TAG in tags(),
        "previous_output_exists": PREV_OUT.exists(),
        "previous_output_pass": prev.get("status") == "PASS",
        "previous_decision_observation_prep_only": prev.get("decision") == "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_OBSERVATION_PACKET_PREPARATION_ONLY",
        ...
        "log_template_field_count": len(template_fields),
        ...
        "observation_field_count": len(OBSERVATION_FIELDS),
        "observation_check_count": len(OBSERVATION_CHECKS),
        "observation_template_field_count": len(obs_template["fields"]),
        "observation_checklist_item_count": len(obs_checklist["checks"]),
        "observation_packet_only": obs_packet["observation_packet_only"] is True,
        "session_execution_not_performed": obs_packet["session_execution_not_performed"] is True,
        "observation_no_real_data": obs_packet["real_data_captured_in_this_phase"] is False,
        "observation_no_candidate_insert": obs_packet["real_candidate_inserted"] is False,
        "observation_no_dataset_acceptance": obs_packet["real_candidate_accepted_to_dataset"] is False,
        "contract_packet_only": contract["observation_packet_only"] is True,
    ...
    result = {
        "status": status,
        "phase": PHASE,
        "decision": "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OBSERVATION_PACKET_READY" if status == "PASS" else "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OBSERVATION_PACKET_NOT_READY",
    ...
    report = [
        "# PROD-4221..4260 Controlled Pilot Manual Dry Run Session Observation Packet",
        "",
        f"- Status: `{status}`",
        ...
        f"- Next: `{obs_packet['recommended_next_phase']}`",
        "",
        "## Boundary",
        "- Observation packet only.",
    ...
    print("next:", obs_packet["recommended_next_phase"])
    print("errors:", errors)
