...
PHASE = "PROD-4341..4380"
REQ_TAG = "product-controlled-pilot-manual-dry-run-session-review-packet-v0.1"

PREV_OUT = ROOT / "outputs/prod4301_4340_controlled_pilot_manual_dry_run_session_review_packet.json"
...
DOC = ROOT / "docs/product/610_CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_REVIEW_READINESS_GATE.md"
CONTRACT = ROOT / "product/contracts/controlled_pilot_manual_dry_run_session_review_readiness_gate.contract.json"
GATE = ROOT / "product/memory/controlled_pilot_manual_dry_run_session_review_readiness_gate_v0_1.json"
CAL_GATE = ROOT / "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_review_readiness_gate_v0_1.json"
OUT_JSON = ROOT / "outputs/prod4341_4380_controlled_pilot_manual_dry_run_session_review_readiness_gate.json"
OUT_MD = ROOT / "outputs/prod4341_4380_controlled_pilot_manual_dry_run_session_review_readiness_gate.md"
...
ALLOWED = [
    "review_readiness_gate_creation",
    "final_gate_packet_preparation",
    "sanitized_review_template_validation",
    "reviewer_final_gate_preflight_validation",
...
def main():
    ...
    decision = "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET_PREPARATION_ONLY"
    ...
    gate = {
        "version": "controlled_pilot_manual_dry_run_session_review_readiness_gate.v0.1",
        "phase": PHASE,
        "decision": decision,
        "purpose": "Validate review packet readiness before preparing final gate packet.",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "review_readiness_gate_only": True,
        ...
        "final_gate_packet_boundary": {
            "next_step_is_final_gate_packet_preparation_only": True,
        ...
        "recommended_next_phase": "PROD-4381..4420 - Controlled Pilot Manual Dry Run Session Final Gate Packet"
    }

    contract = {
        "phase": PHASE,
        "required_prior_tag": REQ_TAG,
        "mode": "controlled_pilot_manual_dry_run_session_review_readiness_gate",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "review_readiness_gate_only": True,
        "final_gate_packet_preparation_only": True,
        "session_execution_not_performed": True,
    ...
    doc = """# PROD-4341..4380 - Controlled Pilot Manual Dry Run Session Review Readiness Gate

Validates the controlled manual dry run session review packet before final gate packet preparation.
...
It approves only preparation of the controlled manual dry run session final gate packet.

Boundary: review readiness gate only. No automatic capture, no real candidate insert, no dataset acceptance, no raw private data, no secrets, no unredacted PII, no production activation and no real-world/client-facing claim.
"""
    ...
    checks = {
        ...
        "previous_review_packet_ready": prev.get("decision") == "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_REVIEW_PACKET_READY",
        ...
        "has_observation_packet_ref": "observation_packet_ref" in review_template_fields,
        "has_observation_template_ref": "observation_template_ref" in review_template_fields,
        "has_observation_checklist_ref": "observation_checklist_ref" in review_template_fields,
        "has_session_log_ref": "session_log_ref" in review_template_fields,
        "has_execution_log_shell_ref": "execution_log_shell_ref" in review_template_fields,
        "has_operator_ref": "operator_ref" in review_template_fields,
        "has_reviewer_ref": "human_reviewer_ref" in review_template_fields,
        "has_source_refs": "source_reference_refs" in review_template_fields,
        "has_privacy_review_ref": "privacy_review_ref" in review_template_fields,
        "has_pii_redaction_ref": "pii_redaction_ref" in review_template_fields,
        "has_secret_scan_ref": "secret_scan_ref" in review_template_fields,
        "has_evidence_packet_ref": "evidence_packet_ref" in review_template_fields,
        "has_manual_observation_refs": "manual_observation_refs" in review_template_fields,
        "has_sanitized_summary_ref": "sanitized_observation_summary_ref" in review_template_fields,
        "has_claim_boundary_review": "claim_boundary_review" in review_template_fields,
        "has_dataset_hold_reason": "dataset_hold_reason" in review_template_fields,
        "has_reviewer_notes_ref": "reviewer_notes_ref" in review_template_fields,
        "has_final_review_gate": "final_review_gate" in review_template_fields,
        "template_status_no_execution": review_template.get("status") == "REVIEW_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "checklist_status_no_execution": review_checklist.get("status") == "CHECKLIST_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "review_packet_only": review_packet.get("review_packet_only") is True,
        "review_execution_not_performed": review_packet.get("session_execution_not_performed") is True,
        "review_no_real_data": review_packet.get("real_data_captured_in_this_phase") is False,
        "review_no_candidate_insert": review_packet.get("real_candidate_inserted") is False,
        "review_no_dataset_acceptance": review_packet.get("real_candidate_accepted_to_dataset") is False,
        "readiness_check_count": len(READINESS_CHECKS),
        "decision_final_gate_packet_prep_only": decision == "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_FINAL_GATE_PACKET_PREPARATION_ONLY",
        "contract_gate_only": contract["review_readiness_gate_only"] is True,
        "contract_final_gate_packet_prep_only": contract["final_gate_packet_preparation_only"] is True,
    ...
    if checks["review_template_field_count"] < 22:
        errors.append("review_template_field_count below 22")
    if checks["review_checklist_item_count"] < 38:
        errors.append("review_checklist_item_count below 38")
    ...
    if checks["readiness_check_count"] < 55:
        errors.append("readiness_check_count below 55")
    ...
    result = {
        "status": status,
        "phase": PHASE,
        "decision": decision if status == "PASS" else "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_REVIEW_READINESS_GATE_NOT_READY",
    ...
    report = [
        "# PROD-4341..4380 Controlled Pilot Manual Dry Run Session Review Readiness Gate",
        "",
        f"- Status: `{status}`",
