...
PHASE = "PROD-4541..4580"
REQ_TAG = "product-controlled-pilot-manual-dry-run-session-execution-precheck-readiness-gate-v0.1"

PREV_OUT = ROOT / "outputs/prod4501_4540_controlled_pilot_manual_dry_run_session_execution_precheck_readiness_gate.json"
PRECHECK_READINESS_GATE = ROOT / "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_execution_precheck_readiness_gate_v0_1.json"
...
DOC = ROOT / "docs/product/615_CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OPERATOR_START_PACKET.md"
CONTRACT = ROOT / "product/contracts/controlled_pilot_manual_dry_run_session_operator_start_packet.contract.json"
SPEC = ROOT / "product/memory/controlled_pilot_manual_dry_run_session_operator_start_packet_v0_1.json"
START_PACKET = ROOT / "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_packet_v0_1.json"
START_TEMPLATE = ROOT / "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_template_v0_1.json"
START_CHECKLIST = ROOT / "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_checklist_v0_1.json"
OUT_JSON = ROOT / "outputs/prod4541_4580_controlled_pilot_manual_dry_run_session_operator_start_packet.json"
OUT_MD = ROOT / "outputs/prod4541_4580_controlled_pilot_manual_dry_run_session_operator_start_packet.md"

OPERATOR_START_SECTIONS = [
    "operator_start_context_refs",
    "prior_gate_chain_review",
    "execution_precheck_readiness_review",
    "precheck_packet_review",
...
OPERATOR_START_FIELDS = [
    "operator_start_packet_id",
    "phase",
    ...
    "precheck_decision_ref",
    "operator_start_decision",
    "operator_start_reason",
    "start_hold_status",
    "next_allowed_phase",
    "stop_before_execution_confirmation"
]

OPERATOR_START_CHECKS = [
    "prior_precheck_readiness_gate_passed",
    ...
    "operator_start_sections_present",
    "operator_start_fields_present",
    "operator_start_checks_present",
    "operator_start_template_fields_present",
    "operator_start_checklist_items_present",
    ...
    "session_execution_plan_preparation_only"
]

ALLOWED = [
    "operator_start_packet_creation",
    "operator_start_template_creation",
    "operator_start_checklist_creation",
    "sanitized_operator_start_placeholder_creation",
...
def main():
    ...
    precheck_gate = read_json(PRECHECK_READINESS_GATE) if PRECHECK_READINESS_GATE.exists() else {}
    precheck_packet_src = read_json(PRECHECK_PACKET) if PRECHECK_PACKET.exists() else {}
    precheck_template_src = read_json(PRECHECK_TEMPLATE) if PRECHECK_TEMPLATE.exists() else {}
    precheck_checklist_src = read_json(PRECHECK_CHECKLIST) if PRECHECK_CHECKLIST.exists() else {}
    ...
    precheck_sections = precheck_packet_src.get("precheck_sections", [])
    precheck_fields = precheck_packet_src.get("precheck_fields", [])
    precheck_checks = precheck_packet_src.get("precheck_checks", [])
    precheck_template_fields = precheck_template_src.get("fields", {})
    precheck_checklist_items = precheck_checklist_src.get("checks", [])
    ...
    start_packet = {
        "version": "controlled_pilot_manual_dry_run_session_operator_start_packet.v0.1",
        "phase": PHASE,
        "purpose": "Prepare controlled manual dry run session operator start packet without executing a session.",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "operator_start_packet_only": True,
        "session_execution_not_performed": True,
        "operator_start_sections": OPERATOR_START_SECTIONS,
        "operator_start_fields": OPERATOR_START_FIELDS,
        "operator_start_checks": OPERATOR_START_CHECKS,
        ...
        "operator_start_boundary": {
            ...
            "sanitized_operator_start_only": True,
        ...
        "claim_boundary": "Operator start packet only. No session execution, no capture, no insert, no dataset acceptance.",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": "PROD-4581..4620 - Controlled Pilot Manual Dry Run Session Operator Start Readiness Gate"
    }

    start_template = {
        "version": "controlled_pilot_manual_dry_run_session_operator_start_template.v0.1",
        "phase": PHASE,
        "status": "OPERATOR_START_TEMPLATE_ONLY_NO_SESSION_EXECUTION",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "fields": {
            ...
            for field in OPERATOR_START_FIELDS
        },
        "default_gate": "HOLD_OPERATOR_START_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }

    start_checklist = {
        "version": "controlled_pilot_manual_dry_run_session_operator_start_checklist.v0.1",
        "phase": PHASE,
        ...
        "real_candidate_accepted_to_dataset": False,
        "checks": [
            {"id": f"START-{i+1:03d}", "name": name, "required": True, "status": "PENDING"}
            for i, name in enumerate(OPERATOR_START_CHECKS)
        ],
        "default_gate": "HOLD_OPERATOR_START_PACKET_NO_SESSION_EXECUTION",
        "blocked_actions": BLOCKED
    }
    ...
    spec = {
        ...
        "operator_start_packet": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_packet_v0_1.json",
        "operator_start_template": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_template_v0_1.json",
        "operator_start_checklist": "product/calibration/real_sessions/controlled_pilot_manual_dry_run_session_operator_start_checklist_v0_1.json",
        "allowed_actions": ALLOWED,
        "blocked_actions": BLOCKED,
        "recommended_next_phase": start_packet["recommended_next_phase"]
    }

    contract = {
        "phase": PHASE,
        "required_prior_tag": REQ_TAG,
        "mode": "controlled_pilot_manual_dry_run_session_operator_start_packet",
        "real_data_captured_in_this_phase": False,
        "real_candidate_inserted": False,
        "real_candidate_accepted_to_dataset": False,
        "operator_start_packet_only": True,
    ...
    doc = """# PROD-4541..4580 - Controlled Pilot Manual Dry Run Session Operator Start Packet

Creates the controlled manual dry run session operator start packet.
...
Boundary: operator start packet only. Use reference-only or sanitized placeholders. No start command, no automatic capture, no raw private data, no secrets, no unredacted PII, no production activation, no client-facing claim, no commercial pricing claim and no validated real-world claim.
"""

    write(DOC, doc)
    write_json(CONTRACT, contract)
    write_json(SPEC, spec)
    write_json(START_PACKET, start_packet)
    write_json(START_TEMPLATE, start_template)
    write_json(START_CHECKLIST, start_checklist)

    checks = {
        "required_tag_present": REQ_TAG in tags(),
        "previous_output_exists": PREV_OUT.exists(),
        "previous_output_pass": prev.get("status") == "PASS",
        "previous_decision_operator_start_prep_only": prev.get("decision") == "APPROVED_FOR_CONTROLLED_MANUAL_DRY_RUN_SESSION_OPERATOR_START_PACKET_PREPARATION_ONLY",
        ...
        "precheck_readiness_gate_exists": PRECHECK_READINESS_GATE.exists(),
        ...
        "operator_start_field_count": len(OPERATOR_START_FIELDS),
        "operator_start_check_count": len(OPERATOR_START_CHECKS),
        "operator_start_template_field_count": len(start_template["fields"]),
        "operator_start_checklist_item_count": len(start_checklist["checks"]),
        "operator_start_packet_only": start_packet["operator_start_packet_only"] is True,
        "session_execution_not_performed": start_packet["session_execution_not_performed"] is True,
        "operator_start_no_real_data": start_packet["real_data_captured_in_this_phase"] is False,
        "operator_start_no_candidate_insert": start_packet["real_candidate_inserted"] is False,
        "operator_start_no_dataset_acceptance": start_packet["real_candidate_accepted_to_dataset"] is False,
        "contract_packet_only": contract["operator_start_packet_only"] is True,
    ...
    if checks["precheck_readiness_check_count"] < 73:
        errors.append("precheck_readiness_check_count below 73")
    ...
    if checks["operator_start_section_count"] < 22:
        errors.append("operator_start_section_count below 22")
    if checks["operator_start_field_count"] < 32:
        errors.append("operator_start_field_count below 32")
    if checks["operator_start_check_count"] < 78:
        errors.append("operator_start_check_count below 78")
    ...
    result = {
        "status": status,
        "phase": PHASE,
        "decision": "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OPERATOR_START_PACKET_READY" if status == "PASS" else "CONTROLLED_PILOT_MANUAL_DRY_RUN_SESSION_OPERATOR_START_PACKET_NOT_READY",
    ...
    report = [
        "# PROD-4541..4580 Controlled Pilot Manual Dry Run Session Operator Start Packet",
        "",
        f"- Status: `{status}`",
        f"- Decision: `{result['decision']}`",
        f"- Operator start sections: `{len(OPERATOR_START_SECTIONS)}`",
        f"- Operator start fields: `{len(OPERATOR_START_FIELDS)}`",
        f"- Operator start checks: `{len(OPERATOR_START_CHECKS)}`",
        ...
        f"- Next: `{start_packet['recommended_next_phase']}`",
        "",
        "## Boundary",
        "- Operator start packet only.",
    ...
    print("operator_start_sections:", len(OPERATOR_START_SECTIONS))
    print("operator_start_fields:", len(OPERATOR_START_FIELDS))
    print("operator_start_checks:", len(OPERATOR_START_CHECKS))
    ...
    print("next:", start_packet["recommended_next_phase"])
    print("errors:", errors)
