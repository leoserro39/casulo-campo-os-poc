#!/usr/bin/env python3
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from api.services.real_intake_engine import process_intake  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CASULO real intake.")
    parser.add_argument("--intake", default=str(ROOT / "real_cases" / "template" / "real_intake.json"))
    parser.add_argument("--write", action="store_true", help="Write evidence manifest/case export to runtime outputs.")
    parser.add_argument("--output-root", default=str(ROOT / "runtime_outputs" / "real_intake"))
    parser.add_argument("--stable-time", action="store_true", default=True)
    args = parser.parse_args()

    result = process_intake(
        Path(args.intake),
        write=bool(args.write),
        output_root=Path(args.output_root),
        stable_time=args.stable_time,
    )
    print(json.dumps(result, indent=2, ensure_ascii=False))
    return 1 if result["status"] != "PASS" else 0


if __name__ == "__main__":
    raise SystemExit(main())
